...
writer = SummaryWriter('runs/ours_128batch_NLL')

# Initialize network
model_path = 'output/sunday/model_12.pth'    #改成最新的
...
trDataloader = DataLoader(trSet,batch_size=batch_size,shuffle=True,num_workers=3,collate_fn=trSet.collate_fn)
valDataloader = DataLoader(valSet,batch_size=batch_size,shuffle=True,num_workers=3,collate_fn=valSet.collate_fn)
#valDataloader = None
...
for epoch in range(pretrainEpochs+trainEpochs):
    ...
    epoch_loss = avg_tr_loss/ num_batch
    writer.add_scalar('Loss/train', epoch_loss, epoch)
    print('epoch:{},avg_loss:{}'.format(epoch, epoch_loss), 'average lat acc:', avg_lat_acc/num_batch, 'average lon acc:', avg_lon_acc/num_batch)
        #save best model
    if epoch_loss < min_loss:
        min_loss = epoch_loss
        torch.save({'epoch': epoch, 'state_dict': net.state_dict()},
                           'output/NLL/model_{}.pth'.format(epoch))
...
'''
    ...
    writer.add_scalar('Loss/val', avg_val_loss/val_batch_count, epoch)
